...
- KTD5. **Pin official egui 0.36.1 plus minimal graph-neutral native fork boundaries.** The egui/eframe fork is based on upstream 0.36.1 and pinned at `4f86e8daa16265e6ca0593622969958c2f50b31f`; the winit event-time fork is pinned at `6105ef864c93c231cad2d76be94054eaf707375a`. Each fork change requires a missing-fact test, a narrow seam, and a deletion or upstream condition. Broad hosted-cycle code is not revived.
...
| --- | --- | --- |
| U1 | The publishable crates use official egui/eframe 0.36.1. The egui/eframe fork is remotely pinned at `4f86e8daa16265e6ca0593622969958c2f50b31f`, the winit fork at `6105ef864c93c231cad2d76be94054eaf707375a`, and clean CI checkouts, manifests, lockfiles, README, and admission documentation agree. | Preserve atomic pin updates and clean-checkout verification when either fork advances. |
...
- **Approach:** Keep official egui/eframe at exact 0.36.1, the egui/eframe fork at `4f86e8daa16265e6ca0593622969958c2f50b31f`, and the winit fork at `6105ef864c93c231cad2d76be94054eaf707375a`. Update each fork manifest, lockfile, CI checkout, and documentation atomically when a pin advances. Verify that each pinned commit contains every API used by native code and that no ignored dirty directory is required. Do not chase unreleased upstream `main`; reconsider the baseline only from a later stable egui tag. Dependency pins are reproducibility inputs; root commit SHAs and worktree digests are not behavior or completion gates.
- **Test scenarios:**
