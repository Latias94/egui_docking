...
- KTD5. **Pin official egui 0.36.1 plus minimal graph-neutral native fork boundaries.** The egui/eframe fork is based on upstream 0.36.1 and pinned at `80c4d12677f40ccd6ce57c8750bada378e6955b8`; the winit event-time fork is pinned at `250a372736afa662d7fffeae5c86a3287b8356c9`. Each fork change requires a missing-fact test, a narrow seam, and a deletion or upstream condition. Broad hosted-cycle code is not revived.
...
| --- | --- | --- |
| U1 | The publishable crates use official egui/eframe 0.36.1. The egui/eframe fork is remotely pinned at `80c4d12677f40ccd6ce57c8750bada378e6955b8`, the winit fork at `250a372736afa662d7fffeae5c86a3287b8356c9`, and clean CI checkouts, manifests, lockfiles, README, and admission documentation agree. | Preserve atomic pin updates and clean-checkout verification when either fork advances. |
...
- **Approach:** Keep official egui/eframe at exact 0.36.1, the egui/eframe fork at `80c4d12677f40ccd6ce57c8750bada378e6955b8`, and the winit fork at `250a372736afa662d7fffeae5c86a3287b8356c9`. Update each fork manifest, lockfile, CI checkout, and documentation atomically when a pin advances. Verify that each pinned commit contains every API used by native code and that no ignored dirty directory is required. Do not chase unreleased upstream `main`; reconsider the baseline only from a later stable egui tag. Dependency pins are reproducibility inputs; root commit SHAs and worktree digests are not behavior or completion gates.
- **Test scenarios:**
