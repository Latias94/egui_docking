...
- KTD5. **Pin official egui 0.36.1 plus minimal graph-neutral native fork boundaries.** The egui/eframe fork is based on upstream 0.36.1; its reviewed local seams still require one immutable remotely reachable revision before the manifest, lockfile, CI, and documentation can name a release baseline. The winit event-time fork is pinned at `180bfc09743586137fec014ef5543cdde56ce5d0`. Each fork change requires a missing-fact test, a narrow seam, and a deletion or upstream condition. Broad hosted-cycle code is not revived.
...
- **Approach:** Keep official egui/eframe at exact 0.36.1 and the winit fork at `180bfc09743586137fec014ef5543cdde56ce5d0`. Publish the reviewed egui/eframe seam to one immutable remote revision, then update the fork manifest, lockfile, CI, and documentation to that same revision in one change. Verify that each pinned commit contains every API used by native code and that no ignored dirty directory is required. Do not chase unreleased upstream `main`; reconsider the baseline only from a later stable egui tag. Dependency pins are reproducibility inputs; root commit SHAs and worktree digests are not behavior or completion gates.
- **Test scenarios:**
