...
- KTD5. **Pin official egui 0.36.1 plus minimal graph-neutral native fork boundaries.** The egui/eframe fork is based on upstream 0.36.1 and pinned at `a35dd9797cb5bdd31f07815190962feebc1e1333`; the winit event-time fork is pinned at `23e8ddc6806a176c63efb80c998a2127faaf03cc`. Each fork change requires a missing-fact test, a narrow seam, and a deletion or upstream condition. Broad hosted-cycle code is not revived.
...
| --- | --- | --- |
| U1 | The publishable crates use official egui/eframe 0.36.1. The egui/eframe fork is remotely pinned at `a35dd9797cb5bdd31f07815190962feebc1e1333`, the winit fork at `23e8ddc6806a176c63efb80c998a2127faaf03cc`, and clean CI checkouts, manifests, lockfiles, README, and admission documentation agree. | Preserve atomic pin updates and clean-checkout verification when either fork advances. |
...
- **Approach:** Keep official egui/eframe at exact 0.36.1, the egui/eframe fork at `a35dd9797cb5bdd31f07815190962feebc1e1333`, and the winit fork at `23e8ddc6806a176c63efb80c998a2127faaf03cc`. Update each fork manifest, lockfile, CI checkout, and documentation atomically when a pin advances. Verify that each pinned commit contains every API used by native code and that no ignored dirty directory is required. Do not chase unreleased upstream `main`; reconsider the baseline only from a later stable egui tag. Dependency pins are reproducibility inputs; root commit SHAs and worktree digests are not behavior or completion gates.
- **Test scenarios:**
