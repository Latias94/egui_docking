...
- U1-U5 foundations and the U6 renderer baseline are implemented. Explicit five-way inner guides, four-way outer guides, all-direction item delivery, and unified contained-title move-or-redock are present.
...
- F2. In-surface docking: a tab or subtree drag opens one session, activates the frontmost complete guide cluster from the current scene, paints every payload-eligible guide, resolves only an exact half-open guide-button or visible tab-gap hit, paints that exact future placement, and commits the same target on the matching release. Pointer presence inside a cluster but outside every button produces a guide-only affordance and no deliverable preview. A contained-title drag freezes its presentation identity, source rectangle, initial pointer, minimum size, and one optional new-presentation identity offer in the core; exact docking always wins, while authoritative local `Known(None)` produces a core-calculated contained move or creation fallback.
...
- F4. Native close: an OS close request creates a frozen full-surface-roster plan without mutating topology and produces a named `CloseDecision`. The plan gives the main root and every contained sibling an explicit atomic disposition. A root-viewport veto maps to one `CancelClose` request; a child-viewport veto keeps submitting the same viewport, while acceptance removes it from the next viewport roster only after the full disposition is valid. Only an enhanced provider's matching destroyed observation may advance `ObservedApplied` and commit the revalidated roster transaction. Dispatch failure, stale incarnation, invalid target, or unavailable recovery host retains the complete logical surface for replacement or later atomic recovery; generic "re-render" is not a close acknowledgement.
...
- F7. Contained floating: an explicit command or opted-in fallback creates a stable contained-floating presentation on a logical surface, clamps it to acknowledged bounds, raises it by explicit focus/z-order facts, and routes move, resize, close-veto, re-dock, and empty-root cleanup through the same engine transaction boundary as tiled docking. A presentation owns an arbitrary split/tabs root; contained presentations remain surface-level siblings rather than recursively nested presentation nodes.
...
- AE12. Given a contained-floating root is moved, resized, focused, veto-closed, or re-docked, then its stable presentation ID and single item ownership are preserved, placement is clamped to acknowledged bounds, z-order changes only from explicit focus input, and an empty presentation is removed atomically. One title-bar drag owns both an exact dock candidate and an exact move fallback: an acknowledged dock target wins, otherwise release commits the last acknowledged translated placement. Existing contained presentations remain movable when creation of new tear-offs is disabled. A new contained presentation reserves identity at most once per drag session, retains that reservation across frames with no fallback, and recomputes geometry from the current pointer instead of reusing stale placement. Complete contained-root drags suppress the smaller tab ghost. No separate dock grip, distance threshold, pointer speed, dwell timer, or modifier inference selects between outcomes.
- AE13. Given a native window whose surface owns a main root and multiple contained siblings is accepted for close or unexpectedly destroyed, then every root has one frozen disposition and the complete roster moves atomically, or the unchanged logical surface remains recoverable for replacement. If the recovery host is not ready, one queryable pending surface roster owns all roots; no single-root partial recovery may claim completion, and content is never orphaned or hidden.
...
- AE15. Given a cross-surface drag whose source cannot be proven pointer-pass-through, then the source binding remains explicit and routing is unavailable rather than being published without a source. Once pass-through has been requested, restoration remains pending across failed, indeterminate, or late results and is cleared only after authoritative observation proves the original input mode; concurrent pointer holders cannot restore early.
- AE16. Given more tabs than the available strip can display at the operable minimum width, then tabs are not compressed into zero-width select or drag regions. Explicit scrolling and overflow selection reveal the selected/focused/dragged tab, hidden tabs publish no hit regions, and every visible closeable tab retains distinct select/drag and close geometry.
...
- **Dependencies:** U2, U3.
- **Files:** `crates/dockspace/src/scene.rs`, `crates/dockspace/src/hit_region.rs`, `crates/dockspace/src/drop_target.rs`, `crates/dockspace/src/drop_guide.rs`, `crates/dockspace/src/drop_resolver.rs`, `crates/dockspace/src/interaction.rs`, `crates/dockspace/src/intent.rs`, `crates/dockspace/tests/drop_resolution.rs`, `crates/dockspace/tests/drop_guide_resolution.rs`, `crates/dockspace/tests/interaction_state_machine.rs`, `crates/dockspace/tests/preview_delivery_parity.rs`.
...
- **Approach:** Define immutable per-generation surface scenes containing node rectangles, tab and tab-bar regions, complete inner/outer guide clusters, independent hit/draw/future-placement rectangles, declared z-order, and policy availability. The cluster type makes missing directions unrepresentable: inner is center plus four edges, outer is four edges, and a single-tabs root publishes one inner cluster whose edge commands split that root rather than a duplicate outer cluster. The engine freezes the pre-frame active-surface roster, emits a non-interactive bootstrap scene for any surface missing acknowledged bounds, and seals exactly once; renderer callbacks cannot add facts to the active generation. Translate responses into source-tagged press/drag/release/cancel/resize inputs and reduce them through the U3 engine boundary. Select the frontmost activated cluster using only explicit layers and structural identity, prevalidate every visible guide for the frozen payload, and resolve only exact half-open button or tab-gap hits. Exact guide buttons take precedence over overlapping tab gaps; an overlapping inner button precedes outer; a rejected geometric winner never falls through. Suppress source-root targets and only its matching contained occlusion exactly when the transaction would remove that complete root, keeping partial and central sources available for local reorder. Carry the exact resolution proof into preview while retaining guide-only affordance separately from paint-acknowledged delivery proof. Delivery requires an authoritative matching button release plus acknowledgement that the same preview was painted; it verifies session, epoch, source, sealed scene, target, and policy before converting the resolution to a workspace command. Tear-off is eligible only for a matching active drag with authoritative `Known(Released)`, painted preview state, authoritative `Known(None)` dock target, known placement, and explicit native or contained presentation policy; `Unknown` or capture loss cancels.
- **Test scenarios:** Complete inner-five and outer-four cluster construction; single-root no-duplicate behavior; nested leaves retain all directions at every root boundary; guide-only activation; all center/left/right/top/bottom commands; half-open button boundaries; inner-over-outer overlap; exact guide-over-tab-gap selection; rejected-winner no-fallthrough; payload-specific center and side availability; complete source-root suppression versus partial and central preservation; unrelated contained occlusion; same-stack reorder; subtree payload; contained-floating occlusion; explicit native versus contained tear-off; `Unknown` target authority or button state; bootstrap, stale, and partially built scenes; every source/target/root callback ordering; missing or closing target surface; late fact after seal; duplicate release; release without active session; source removal mid-drag; capture loss; new drag replacing old drag; resize cancellation; preview target changed before delivery; successful and failed restore during active drag with old-generation inputs rejected.
...
**2026-07-19 hardening amendment:** Drag-source binding is frozen independently of pointer-hit-test control, and routing requires authoritative source pass-through observation. Pass-through uses an explicit multi-holder lease whose restoration obligation survives dispatch failure and indeterminate or late results until the original input mode is observed. Native close and destruction freeze the complete `main_root + contained roots` roster and either commit one atomic disposition transaction or retain one recoverable logical surface for replacement. The required matrix adds unsupported hit-test control, late/failed enable, failed/retried restore, original pass-through, multiple pointer holders, and a split main root plus multiple contained roots through accepted close, unplanned destruction, unavailable recovery, replacement, veto, and stale source.
...
- **Approach:** Expose an application-owned pane registry keyed by core item IDs and a facade that advances the core frame around egui painting. Submit acknowledged facts and fixed guide/style metrics before scene construction, paint only a complete sealed scene and already resolved preview, queue source-tagged semantic inputs during each immediate viewport callback, then reduce them after all callbacks for the next generation. Geometry learned during paint cannot alter the generation being painted. Paint passive and active guide buttons above the body preview, with directional cues and unavailable state, while only the body preview participates in paint acknowledgement. Implement tab selection/close/reorder, subtree drag, splitter resize, central area, empty states, drag ghost, and the full contained-floating flow: explicit creation/fallback selection, bounds clamping, deterministic focus/z-order, title-bar move-or-redock as one proof-bearing drag, resize, close veto, and empty cleanup. Freeze the source rectangle and pointer offset at drag start; a resolved dock preview wins, otherwise the same drag retains the last painted contained placement. Implement arrow/Home/End tab navigation, Enter/Space activation, policy-checked close, Escape drag cancellation, keyboard splitter adjustment, and AccessKit semantics from the sealed scene. Keep advanced core access available without duplicating state.
- **Test scenarios:** Builder creates canonical layouts; visible five-way inner and four-way outer guide matrices; dragging a right-panel tab onto a left panel resolves and delivers center, left, right, top, and bottom exactly; guide-only versus active versus unavailable paint states; a contained title drag moves when no guide is active and re-docks on every exact guide without a separate grip; stale or rejected guides never fall back to an unseen move; existing contained movement with new tear-off disabled; one drag leaving, re-entering, and leaving the host reuses one reserved identity while publishing only the latest placement; missing pane registry entry renders a recoverable placeholder; tab close veto; selected tab focus; same-stack reorder; split resize constraints; every contained-floating transition in AE12; explicit fallback off/on; style metric changes; pane callback adds/removes content safely through next-boundary commands; keyboard/pointer command parity; accessibility selected/closeable/focused/adjustable assertions; adapter never changes graph during paint.
...
| U4 | Complete sealed guide clusters, guide-only affordance, exact-button resolution, rejected-target non-fallthrough, callback-order matrices, drag generation, authoritative one-release semantics, and painted-preview/delivery parity pass without default heuristics |
| U5 | Fake-platform viewport saga, indeterminate-effect, full-surface-roster close/recovery, fail-closed source routing, durable pass-through restoration, compensation, stale-result, and mixed-DPI suites pass |
| U6 | Public egui API, visible five-way/four-way docking guides, all-direction and complex-floating egui gesture tests, core-owned unified contained title move-or-redock, deterministic tab overflow, renderer, complete contained-floating flow, keyboard/accessibility semantics, persistence helpers, and upstream-only examples pass |
