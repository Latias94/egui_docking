...
- F2. In-surface docking: a tab or subtree drag opens one session, activates the frontmost complete guide cluster from the current scene, paints every payload-eligible guide, resolves only an exact half-open guide-button or tab-gap hit, paints that exact future placement, and commits the same target on the matching release. Pointer presence inside a cluster but outside every button produces a guide-only affordance and no deliverable preview.
...
| U6 | Public egui API, visible five-way/four-way docking guides, all-direction egui delivery tests, unified contained title move-or-redock, renderer, complete contained-floating flow, keyboard/accessibility semantics, persistence helpers, and upstream-only examples pass |
