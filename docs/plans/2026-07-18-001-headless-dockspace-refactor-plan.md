...
- **Verification:** Fake-provider tests cover protocol branches but make no product claim; crates.io examples remain single-surface/contained and expose no runnable native multiview facade. `egui_dockspace_native` is dependency-direction checked so neither `dockspace` nor `egui_dockspace` depends on it and no fork crate imports docking graph types. Exactly one real-window smoke proves the production event-loop path from outside-all tear-off through first-live admission and cross-window redock. Close, focus, recovery, pass-through compensation, mixed-DPI, ABA, failure, and ordering matrices remain deterministic Rust protocol/host tests rather than additional real-window scenarios. The smoke asserts only the semantic ownership and presentation barriers needed to prove that vertical slice; it is not a scenario runner, screenshot suite, or platform matrix. The fork diff is graph-agnostic, typed, tested, based on the exact upstream `0.35.0` tag, and split into focused event-correlation, receiver-journal, presentation-result, and renderer-lifecycle commits. Native Git manifests and harnesses name its full remotely resolvable revision; crates.io manifests contain no fork dependency. Git-native full-support claims remain gated on the authorized pinned remote revision and this smoke plus deterministic conformance; crates.io full-support claims additionally require publishable registry seams.
...
| Public API seal | Build strict public rustdoc, compile one downstream allowed-surface fixture, and inspect both crate roots; adopt an established semver/public-API tool only after the facade is intentionally stable | Internal scene, drag, route, effect, compiler, resolver, and lifecycle FSM modules are private before independent conformance; `egui_dockspace` has no glob core re-export; no repository runner parses Rust, infers call graphs, or invents an API safety classification | U6, U9, U8 |
...
| Fork integration gate | Run targeted egui-winit/eframe tests and the thin excluded fork/native Cargo harness | Fork seams compile and their deterministic protocol tests pass against one pinned egui source; the runner contains no product semantics, source parser, digest protocol, or scenario engine | U7 |
| Native event-loop smoke | Run exactly one two-window smoke through the production event loop | Outside-all tear-off crosses first-live admission and redocks without pane loss; all other lifecycle and platform permutations stay in deterministic Rust tests | U7 |
