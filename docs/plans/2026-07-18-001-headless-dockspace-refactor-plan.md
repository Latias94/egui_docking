...
- AE12. Given a contained-floating root is moved, resized, focused, veto-closed, or re-docked, then its stable presentation ID and single item ownership are preserved, placement is clamped to acknowledged bounds, z-order changes only from explicit focus input, and an empty presentation is removed atomically. One title-bar drag owns both an exact dock candidate and an exact move fallback: an acknowledged dock target wins, otherwise release commits the last acknowledged translated placement. No separate dock grip, distance threshold, pointer speed, dwell timer, or modifier inference selects between them.
