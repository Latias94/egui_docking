...
```mermaid
flowchart LR
  S[Open GPUI source test] --> C[Headless core trace]
...
| --- | --- | --- |
| `core-trace` | Typed deterministic trace run directly against the headless core. | Graph, transaction, policy, lifecycle, and failure semantics only. |
...
### Treat core traces as complete conformance

Rejected. `dockspace_core_protocol` directly owns `DockEngine`; it proves core
reducer behavior but cannot prove adapter ordering, real receiver ownership, or
OS lifecycle facts.
...
| --- | --- | --- |
| Catalog completeness | 12/12 cases have provenance and required executor assignments. | This document and fixture metadata review. |
| Core parity | Every `core-trace` case passes deterministically. | Typed trace replay in CI. |
