//! Internal failure details produced by the egui facade.
...
/// Exact adapter/backend diagnostic retained behind [`DockspaceError`].
#[derive(Debug, Error)]
...
impl DockspaceErrorSource {
    pub(crate) const fn kind(&self) -> DockspaceErrorKind {
        match self {
            #[cfg(feature = "serde")]
            Self::DocumentSession(_) => DockspaceErrorKind::Persistence,
            Self::Style(_) | Self::PresentationConfig(_) => {
                DockspaceErrorKind::InvalidConfiguration
            }
            Self::SingleSurfaceHostFrameRequiresOneSurface { .. } => {
                DockspaceErrorKind::Unsupported
            }
            #[cfg(any(feature = "backend", test))]
            Self::MultiSurfaceHostFrameUnsupported { .. } => DockspaceErrorKind::Unsupported,
            Self::ContainedPlacement(_)
            | Self::NativeSessionAlreadyActive
            | Self::BackendApplicationInputRequiresIngress
            | Self::PointerInputProviderAlreadyInstalled
            | Self::PointerInputControlDuringPendingEpoch
            | Self::PointerInputFrameInFlight => DockspaceErrorKind::OperationConflict,
            Self::NativeBinding(_)
            | Self::NativeBindingRegistryUnavailable
            | Self::NativeSurfaceBindingChanged { .. }
            | Self::NativeSurfaceOutputWithoutCallback { .. }
            | Self::NativeCallbackViewportMismatch { .. }
            | Self::NativeReceiverOutputSurfaceMismatch { .. }
            | Self::NativeStagingRequestUnavailable { .. }
            | Self::NativeStagingBindingMismatch { .. }
            | Self::NativeStagingRequestOutsideRoster { .. }
            | Self::NativePresentationEndpointMismatch { .. }
            | Self::NativeSessionLeaseMismatch
            | Self::BackendIngress(_)
            | Self::HostFrameNotIncreasing { .. }
            | Self::HostFrameSurfaceOutsideRoster { .. }
            | Self::HostFrameDuplicateSurface { .. }
            | Self::HostFrameSurfaceViewportChanged { .. }
            | Self::HostFrameSurfacePassNotIncreasing { .. }
            | Self::OuterHostFrameRequired
            | Self::OuterHostSurfaceContextMismatch { .. }
            | Self::OuterHostSurfaceViewportMismatch { .. }
            | Self::OuterHostSurfaceOutputPassMismatch { .. }
            | Self::OuterHostSurfaceOutputPassExhausted { .. }
            ...
            | Self::CoreHostFrame(_)
            | Self::SemanticActionCausalityUnavailable { .. }
            | Self::CrossViewportSemanticInputRequiresBackendAuthority { .. }
            | Self::PointerJournal(_)
            | Self::SurfaceLocalPointerProvider(_)
            | Self::PointerReceiverObservation(_)
            | Self::HostFrameHoverDropResolution(_)
            | Self::TabInteractionUnavailable(_)
            | Self::PointerReceiverReceiptBatch(_)
            | Self::PointerInputEpochAdvancedBeforeCommit
            | Self::PointerInputBindingMissing
            | Self::PointerInputBindingStale
            | Self::PointerInputAbortFailed { .. }
            ...
            | Self::Engine(_)
            | Self::Projection(_)
            | Self::AutomaticHostFrameSequenceExhausted
            | Self::AutomaticPresentationCaptureGenerationExhausted { .. }
            | Self::OuterPresentationCaptureGenerationExhausted { .. }
            | Self::NativeStyleConfigurationNotAccepted { .. }
            | Self::PresentationPhaseProducedSemanticInput { .. }
            | Self::InputSourceSequenceExhausted { .. }
            | Self::PaintResourceMissing { .. }
